...
WorkBoard is a local, single-user tool. Board data is plain files in `~/.workboard` (every board lives under `~/.workboard/boards/`), and the server is meant to be reached only by your own browser.
