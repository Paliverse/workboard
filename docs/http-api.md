...
```json
{"ok": true, "app": "workboard", "version": "0.1.0", "apiVersion": 2, "schemaVersion": 2,
 "supportedSchemaVersions": [1, 2], "capabilities": ["context", "..."],
...
| `GET api/card/{ref}/context` | `{"ok", "board", "schemaVersion", "rev", "card", "dependencies": [{"id", "num", "title", "column", "outcome", "satisfied"}], "missingDependencies", "dependents": [{"num", "id", "title", "column", "outcome"}], "ready", "omitted"?}`. The card is trimmed to the 10 most recent done subtasks and the last 25 history entries; `omitted` reports what was trimmed. |

### `POST board.json`: snapshot save

```jsonc
{ "baseRev": 12, "actor": "user", "schemaVersion": 2,
...
- Server-owned card fields may be sent only unchanged (422 otherwise): `id`, `num`, `createdAt`, `updatedAt`, `history`, `changedRev`, `activeOwner`, `claimedAt`, `dependsOn`, `outcome`, `cancelReason`, `reworkReason`, `reopenReason`, `blockedReason`, `unblockWhen`, `blockedAt`, `doneAt`, `comments`, `attachments`, `cycles`, `verification` and `reviews`.
...
- `card` is required. Other keys in it are dropped, and server-owned fields must be unchanged.
...
Body: `{"baseRev", "actor"?, "action", "details": {}}`. If the card has another owner, every action except `takeover` and `workpad` is 409. Required text is trimmed and must not be blank.
...
| `workpad` | | any | Adds missing acceptance and verification sections to the notes. |
...
The document (`schemaVersion` 2) contains `name`, `title`?, `rev`, `nextNum`, `savedAt`, `savedBy`, `columns` and `cards` (array order is display order), plus optional `tagTaxonomy`, `activeWork` and `activeWorkId`. Unknown top-level keys on disk are preserved. Timestamps are UTC `YYYY-MM-DDTHH:MM:SSZ`.
...
| `code`, `title`, `origin`, `notes`, `writeup` | Editable strings |
