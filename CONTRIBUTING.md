...
Keep development runs away from your real boards, registry and service. Point `WORKBOARD_HOME` at a scratch directory, and use a free port instead of 7891:

```sh
export PYTHONPATH=src WORKBOARD_HOME="$PWD/build/dev-home" WORKBOARD_PORT=7990
python -m workboard init demo --dir build/demo
python -m workboard serve      # http://127.0.0.1:7990/b/demo/
```

Never run `setup`, `skills install` or `service install` from a development checkout unless you mean to replace your installed skill and service.
...
| Area | Module |
|---|---|
| CLI, schema, locking, backups, recovery, registry | `tests.test_cli` |
...
- **Scratch homes only.** Every test runs against a scratch `HOME`, `USERPROFILE`, `WORKBOARD_HOME`, `APPDATA`, `LOCALAPPDATA` and `XDG_*` from `tests/support.py` (`scratch()`, `make_env()`, `run()`, `last_json()`). A test must never read or write the real user's registry, skills or service registrations.
- **Test servers** listen on port 0 or a free ephemeral port and are always stopped.
- **Test contracts, not implementation.** A test should fail on a plausible bug in observable behavior: output, exit codes, persisted data, HTTP responses or conflicts.
...
- Update the docs in `docs/` and `README.md` when behavior changes. Add a line under `## [Unreleased]` in [CHANGELOG.md](CHANGELOG.md).
